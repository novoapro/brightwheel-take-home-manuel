...
Everything is built from a few atomic entities. The **PolicyRecord is the source of truth** — the citable unit. The **parent-facing handbook is a read-only view *derived from* PolicyRecords** (DECIDED), so there is exactly one source of truth and the "per the handbook" trust cue can't drift from what the assistant cites.
...
>
> Exact schedules, tuition numbers, holiday calendar, illness thresholds, and lunch menu are authored as PolicyRecords during the seed stage.

### 2.2 `PolicyRecord` — the atomic, citable source of truth
Each record answers one coherent thing, self-contained (no chunking needed).

```
PolicyRecord {
  id                 // stable, human-readable e.g. "hours.holidays.2026"
  intent             // hours | tuition | health | meals | tours   (the 5)
...
### 2.3 `Escalation` — an unknown/sensitive question relayed to staff (real-time)
```
Escalation {
  ...
  operator_answer, answered_by, answered_at,   // relayed LIVE into the parent thread
  promoted_policy_id     // set when the answer becomes a PolicyRecord (capture)
  question_embedding     // to dedupe/cluster incoming unknowns
}
```
The staff answer relays into the same parent conversation in real time (one voice — see [03 §3.3](03-ux-flows.md)); it is **not** an async callback.

### 2.3b Message provenance
Every assistant-side chat message carries `provenance: "grounded" | "staff"` — `grounded` = AI answered from cited PolicyRecords (📎), `staff` = a person answered/confirmed in real time (👤). This is the trust spine of the parent UI and lets metrics ([05](05-quality-audit-and-metrics.md)) distinguish AI-deflected from staff-answered.

### 2.4 `InteractionAudit` — every parent turn (from Stage 05)
The immutable log defined in [05-quality-audit-and-metrics.md](05-quality-audit-and-metrics.md) §2. Foreign-keys to any PolicyRecords cited and to an Escalation if one was raised. This is what all metrics compute over.
...
Settings {
  ...
  active_provider: "claude" | "gemini"               // the A/B toggle ([03 §4.4](03-ux-flows.md))
}
```
...
```
Center 1─┐
         └─* PolicyRecord ─cited_by─* InteractionAudit *─raises─0..1 Escalation
                    ▲                                                   │
                    └──────────────── promoted_to (capture) ───────────┘
```
The loop is literally an edge in the schema: an `Escalation.operator_answer` becomes a new `PolicyRecord(origin="captured")`, which then gets `cited_by` future interactions. **The compounding-deflection story is a foreign key.**
...
2. Candidate select:
     a. Structured filter: PolicyRecords WHERE intent = X AND status=published
...
5. Attribution: cite the PolicyRecord.id(s) the answer used → render source chip
...
- **Embeddings** (`PolicyRecord.embedding`, `Escalation.question_embedding`) would power the *curation* side — cluster top gaps, dedupe repeat unknowns, check for near-duplicate policies on capture. **Reserved in schema, unpopulated in v1;** in v1 the curation view uses intent + keyword grouping instead.

---

## 4. Attribution model (trust is the product)

- Every answer carries **explicit citations to PolicyRecord ids**, surfaced as a "per the handbook: … (Family Handbook p.4)" chip.
...
- **~15–25 PolicyRecords** across the 5 intents, each with real `structured` payloads (a holiday calendar, a tuition table, illness thresholds, a lunch menu + allergy policy, a tour process).
...
parent asks unknown Q
  └─ InteractionAudit{decision: escalated} ─raises→ Escalation{status: waiting}
        └─ staff answers (relayed live) → Escalation{status: answered, operator_answer}
              └─ promote     → PolicyRecord{origin: captured}   (embedding deferred in v1)
...
3. **Handbook:** ✅ **Derived read-only view from PolicyRecords** — single source of truth.
