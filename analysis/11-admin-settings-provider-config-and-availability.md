...
| **B — Online/Away + operator identity** (§4) | Flips the Front Desk **Online** or **Away** from a header CTA (manual, or follow center hours), **identifying the on-duty operator** — a name required to go Online, persisted center-wide. | **Online:** live indicator naming the operator ("🟢 Maria is at the front desk") + real-time staff relay. **Away:** "away" indicator + disclaimer; escalations become **email follow-ups** (async), not live relay. Live answers are attributed to the named operator. |
...
- `Settings { caution_level, active_provider: "claude" | "gemini" }` ([src/lib/repo/settings.ts](../src/lib/repo/settings.ts)); default provider `claude`.
...
```
**Exactly one provider is active at a time** (the requirement). The admin selects a provider, then configures *that* provider's key + model. Per-provider config is persisted (so switching back doesn't force re-entry), but only `active_provider` is used to route calls.
...
| **Google** | `@google/genai` (present) | Gemini Flash / Pro (confirm current IDs at build) | Gemini Flash | Gemini Flash |
...
1. **Encrypt at rest — AES-256-GCM** via Node's built-in `crypto` (no new dep). A 32-byte master key from env **`SECRETS_ENCRYPTION_KEY`** (base64) encrypts each provider key; store `iv‖authTag‖ciphertext` (base64) in the DB. GCM's auth tag detects tampering. If `SECRETS_ENCRYPTION_KEY` is unset, provider-key config is **disabled** (falls back to env keys) rather than storing plaintext — fail safe.
...
- **`getModel()` resolves credentials.** Signature becomes `getModel(): Promise<FrontDeskModel>` (or takes a resolved `ProviderConfig`): it reads `active_provider`, loads + **decrypts** that provider's key and model id from the credential repo, and constructs the client with an explicit key (e.g. `new Anthropic({ apiKey })`, not the env default). **Env keys remain a bootstrap fallback** when no DB key is configured — so local dev and the seed demo keep working.
...
└──────────────────────────────────────────────┘
```
- Switching the radio loads that provider's saved config (masked). Save persists key (if changed) + model + sets it active; a spinner runs the liveness check.
...
```ts
Settings.availability: "online" | "away"        // manual toggle (v1)
Settings.availability_mode?: "manual" | "hours" // "hours" = auto from center hours (enhancement)
...
```
- **v1 = a manual Online/Away toggle** (in the header CTA — §4.2a — and mirrored in Settings). **Enhancement (fork):** `hours` mode derives status from the center's `hours_general` + timezone ([01 §2.1]) so it flips itself at open/close.
...
- **One coupled fact — presence + identity.** Model availability and the on-duty operator as a single center-wide truth: *"Little Acorns' Front Desk is Online, staffed by Maria."* `operator_name` lives on the single `Settings` row (survives reloads and device changes; shared across admin browsers — coherent because presence is itself a center-wide fact). **Invariant: `availability="online"` requires a non-empty `operator_name`** — enforced server-side in the settings PUT, not just the UI, so the desk is never open anonymously and every live answer is attributed. `operator_name` **persists across Away↔Online** (Away does not clear it — re-opening is one tap; the parent only *sees* the name while Online).

### 4.2 Parent UI — indicator + disclaimer
A small status pill in the parent header ([FrontDesk.tsx](../src/components/FrontDesk.tsx)), server-provided so there's no flash. The server component ([src/app/page.tsx](../src/app/page.tsx)) reads `getSettings(getDb())` alongside `getCenter` and passes `availability` + `operator_name` as props — the same pattern that already feeds `center` into the client component:
- **Online:** 🟢 **"{operator_name} is at the front desk — usually replies in real time."** (e.g. *"Maria is at the front desk…"*). Naming the human on duty is a warmth/trust win — the parent sees a real person is present, not a bot. Behavior = today (grounded answers + live staff relay).
- **Away:** 🟡 "Away" + a one-line disclaimer banner (no name shown):
  > *"We're away right now. I can answer common questions from our handbook, but for anything I'm unsure about, leave your email and our team will follow up — usually within one business day."*

Honesty first ([00 §Trust]): the disclaimer sets expectations *before* the parent asks, so an escalation feels like a promise kept, not a dead end. Naming the operator only while Online is the same honesty in reverse — we surface a person exactly when one is actually reachable.
...
- **Relationship to Settings → Availability (§4.5):** the header pill is the **quick toggle** (open/close + who's on, one tap, always in view); the Settings → Availability section stays the **richer config** (away note, `hours` mode). Both write the same `Settings` fields and must stay consistent.
...
- When the operator answers a `delivery:"email"` escalation in the relay queue, the answer routes through `EmailSender` (not the SSE bus) and the escalation is marked `answered` + `delivered`. **Capture-to-policy still applies** ([03 §4.2]) — a general Q&A answered by email can still become a citable PolicyRecord, so deflection compounds even off-hours.

### 4.5 Admin UI — Settings → "Availability" + the relay queue
```
┌ Availability ──────────────────────────────┐
│ Status   (•) Online   ( ) Away              │
│ Mode     (•) Manual   ( ) Follow center hours│  ← enhancement
│ Away note[ We're away right now… ]          │
└──────────────────────────────────────────────┘
```
- This section mirrors the header presence CTA (§4.2a) — same `availability` field, so flipping in either place is equivalent. The **quick toggle + operator name lives in the header** (always visible); this section keeps the richer config (away note, `hours` mode).
...
| File | Change |
|---|---|
| [src/lib/types.ts](../src/lib/types.ts) | `Provider` → `anthropic\|openai\|google`; `Settings` gains `availability`, `availability_mode?`, `away_message?`, **`operator_name?`**; `Escalation` gains `delivery`, `contact_*`, `delivered_at`. |
...
| [src/app/page.tsx](../src/app/page.tsx) | Server component reads `getSettings(getDb())` alongside `getCenter`; pass `availability` + `operator_name` as props into `<FrontDesk>`. |
| [src/components/FrontDesk.tsx](../src/components/FrontDesk.tsx) | Online/Away pill (Online names the operator — §4.2) + disclaimer; inline contact-capture on away-escalation. |
...
- **API keys:** encrypted at rest, write-only API, masked reads, redacted logs, decrypt only in-process at call time, gated by passcode + a server-only master key. Fail safe (no master key → no plaintext storage).
...
- **Online/Away + operator identity** is deep **user empathy** and **warmth/trust**: it never pretends a human is there when one isn't, names the real person when one is ("Maria is at the front desk"), and still resolves the question (async email) instead of dead-ending — the escalation loop that *teaches* ([00 §4]) keeps compounding even off-hours. Persisting the operator name also removes daily friction for the busy owner (no re-typing) — *less-is-more scrappiness*.
...
3. **AES-256-GCM at rest** (Node `crypto`, `SECRETS_ENCRYPTION_KEY`), write-only/masked API, decrypt at the edge, env fallback, fail-safe.
