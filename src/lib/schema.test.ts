...
describe("schema / migrate", () => {
  ...
  it("enforces the capture-edge FK (escalation.promoted_policy_id → policies)", () => {
    const db = createMemoryDb();
    expect(() =>
      db
        .prepare(
          `INSERT INTO escalations (id, question, reason, promoted_policy_id, created_at)
           VALUES ('e1', 'q', 'out_of_scope', 'ghost.policy', '2026-01-01')`,
        )
        .run(),
    ).toThrow(/FOREIGN KEY/i);
  });
      ...
      db
        .prepare(
          `INSERT INTO policies (id, intent, title, body_md, updated_at)
           VALUES ('p', 'nonsense', 't', 'b', '2026-01-01')`,
        )
        .run(),
    ).toThrow(/CHECK/i);
    // bad decision
